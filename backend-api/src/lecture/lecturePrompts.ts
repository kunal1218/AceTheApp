...
export function buildGeneralLectureAnswerToLectureJsonPrompt(
  ...
  return [
    `You will be given an answer text. Treat it as the source of truth.`,
    `Your job is to make it SOUND like a teacher speaking while keeping the substance identical, then convert it into STRICT JSON.`,
    ``,
    `NON-NEGOTIABLE FIDELITY RULES:`,
    `- Do NOT add new technical claims, new facts, or new examples.`,
    `- Do NOT remove any substantive point that appears in the answer.`,
    `- Do NOT invent structure that is not already implied by the answer.`,
    `- Do NOT introduce "Part 1/Part 2" sections or generic filler transitions.`,
    `- Keep the same idea order as the answer:`,
    `  (1) what it is → (2) why it matters → (3) misconceptions → (4) edge cases,`,
    `  with the worked example where it originally appears (do not move it far).`,
    `- Keep EXACTLY ONE worked example. Do not create additional examples or extra numeric variants.`,
    `- Do NOT include course references, tie-ins, assignments, professors, or dates.`,
    `- Do NOT include topQuestions.`,
    ``,
    `Step A (internal rewrite only — do not output separately):`,
    `Rewrite the answer into a teacher-spoken style by doing ONLY these operations:`,
    `- Rephrase sentences to sound spoken (same meaning).`,
    `- Add short, natural teacher transitions (1 sentence max per transition).`,
    `- Clarify pronouns/referents when needed for readability (without adding facts).`,
    `Prohibited in Step A:`,
    `- Headers, bullets, numbered lists, "Part X", "Definition and Motivation" labels.`,
    `- Meta/scaffolding phrases like:`,
    `  "With that foundation in place", "We start by", "In this section", "Part X focuses on".`,
    `- Repeating the same sentence pattern with different numbers.`,
    ``,
    `Step B (output JSON only): Convert the rewritten teacher-spoken version into STRICT JSON matching the schema below.`,
    `JSON RULES:`,
    `- Return ONLY valid JSON. No markdown. No commentary.`,
    `- Create 5 to 10 chunks.`,
    `- Each chunk must include: chunkTitle, narration.`,
    `- narration must preserve the answer’s substance; light rephrasing is allowed ONLY for teacher voice.`,
    `- Chunk titles should be short, content-grounded (no "Part 1", no generic boilerplate).`,
    `- The set of chunks must collectively cover the answer in the same order.`,
    ``,
    `WHITEBOARD RULES:`,
    `- Identify the single worked example from the original answer.`,
    `- Add whiteboardOps ONLY in the chunk that contains that worked example.`,
    `- No other chunk may contain whiteboardOps.`,
    `- whiteboardOps must reflect ONLY what the worked example already does (no extra steps or extra examples).`,
    ``,
    `Schema (must match exactly):`,
    `${schemaHint}`,
    ``,
    `Answer text (source of truth):`,
    `<<<BEGIN_ANSWER`,
    call1AnswerText,
    `END_ANSWER>>>`,
    ``,
